...
class LocalDecoder6D(nn.Module):
    ''' Decoder.
    ...
    def __init__(self, dim=6, c_dim=128, out_dim=1,
    ...
    def forward(self, p, c_pos, return_desc=False, **kwargs):
        '''
        Args:
            p (b, m, 3) query points
            c_pos (b, scene_feat_dim, x, y, z) the grid features
        Returns:
            out: (b, m, out_dim) decoded result
            c: (b, m, scene_feat_dim) sampled feature from the implicit scene representation
        '''
        if self.c_dim != 0:
            plane_type = list(c_pos.keys())
            c = 0
            if 'grid' in plane_type:
                c += self.sample_grid_feature(p, c_pos['grid'])   # c (b, feat_dim, m)
            c = c.transpose(1, 2)   # (b, m, feat_dim)
