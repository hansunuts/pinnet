...
from models.common import coordinate2index, normalize_coordinate, normalize_3d_coordinate, map2local
from models.layers.unet import UNet
from models.layers.unet3d import UNet3D
...
class LocalPoolPointnet(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
        ...
        if unet3d:
            self.unet3d = UNet3D(in_channels=c_dim, out_channels=c_dim)
        else:
            self.unet3d = None
    ...
    def generate_grid_features(self, p, c):
        ...
        fea_grid = fea_grid.reshape(p.size(0), self.c_dim, self.reso_grid, self.reso_grid, self.reso_grid) # sparce matrix (B x c_dim x reso x reso x reso)
    ...
    def forward(self, p):
        ...
        fea = {}
        if 'grid' in self.plane_type:
            fea['grid'] = self.generate_grid_features(p, c) # [b, feat_dim, x, y, z]
        if 'xz' in self.plane_type:
